...
Module modified from teaser package
_author_: Daniele Schiera daniele.schiera@polito.it

"""
import sys
#sys.path.append("teaser_modified/")
import warnings
import os
import re
...
import teaser_modified.aixlib_out as aixlib_output
import teaser_modified.ibpsa_out as ibpsa_output
    ...
    def export_aixlib(
        ...
        if internal_id is None:
            aixlib_output.export_multizone(
                buildings=self.buildings, prj=self, path=path, fmu_io = fmu_io, template_path=template_path
            )
        else:
            for bldg in self.buildings:
                if bldg.internal_id == internal_id:
                    aixlib_output.export_multizone(
                        buildings=[bldg], prj=self, path=path, fmu_io = fmu_io, template_path = template_path
                    )
        return path


    def export_ibpsa(self, library="AixLib", internal_id=None, path=None,fmu_io= False, template_path=None): # Added FMU_io to custom input
        # and output signals for FMU functionalities
        """Exports values to a record file for Modelica simulation
        ...
        if internal_id is None:
            ibpsa_output.export_ibpsa(
                buildings=self.buildings, prj=self, path=path, library=library, fmu_io= fmu_io, template_path= template_path)

        else:
            for bldg in self.buildings:
                if bldg.internal_id == internal_id:
                    ibpsa_output.export_ibpsa(buildings=[bldg], prj=self, path=path, fmu_io= fmu_io, template_path= template_path)
        return path
