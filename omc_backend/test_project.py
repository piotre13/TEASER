...
internal_gains_mode: int [1, 2, 3]
    ...
    def export_ibpsa(self, library="AixLib", internal_id=None, path=None,fmu_io= False): # Added FMU_io to custom input
        ...
        if internal_id is None:
            ibpsa_output.export_ibpsa(
                buildings=self.buildings, prj=self, path=path, library=library, fmu_io= fmu_io
            )
        else:
            for bldg in self.buildings:
                if bldg.internal_id == internal_id:
                    ibpsa_output.export_ibpsa(buildings=[bldg], prj=self, path=path, fmu_io= fmu_io)
        return path
