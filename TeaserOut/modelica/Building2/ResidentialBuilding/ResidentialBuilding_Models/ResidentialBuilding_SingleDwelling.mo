within Building2.ResidentialBuilding.ResidentialBuilding_Models;
model ResidentialBuilding_SingleDwelling
  ...
  AixLib.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    TDryBulSou = AixLib.BoundaryConditions.Types.DataSource.Input,calTSky=AixLib.BoundaryConditions.Types.SkyTemperatureCalculation.TemperaturesAndSkyCover,
    ceiHeiSou = AixLib.BoundaryConditions.Types.DataSource.Input,
    computeWetBulbTemperature=false,
    filNam=Modelica.Utilities.Files.loadResource("modelica://Building2/USA_CA_San.Francisco.Intl.AP.724940_TMY3.mos"),
  ...
  parameter Real l_cooler=-167500 "Lower limit controller output of the cooler";
  parameter Real KR_cooler=100 "Gain of the cooling controller";
  parameter Modelica.SIunits.Time TN_cooler=10000 "Time constant of the cooling controller";
  ...
  parameter Building2.HeaterCooler_param zoneParam() "NOTA: fake zoneParam only used only for HeaterCooler param";
    ...
    AixLib.Utilities.Sources.HeaterCooler.HeaterCoolerPI heaterCooler(
     ...
     l_heater = l_heater,
     recOrSep = recOrSep,
     staOrDyn = true,zoneParam=zoneParam) if (not recOrSep and (Heater_on or Cooler_on)) "Heater Cooler with PI control"
    annotation (Placement(visible = true, transformation(extent = {{-22, -102}, {4, -76}}, rotation = 0)));
  AixLib.Utilities.Sources.HeaterCooler.HeaterCoolerController heaterCoolerController(zoneParam = zoneParam) if withIdealThresholds
    annotation (Placement(visible = true, transformation(origin = {-66, -94}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealInput heatCoolRoom_in(start=0) annotation(
    Placement(visible = true, transformation(origin = {152, -160}, extent = {{-20, -20}, {20, 20}}, rotation = 90), iconTransformation(origin = {-108, -36}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow heatCoolRoom annotation(
    Placement(visible = true, transformation(origin = {152, -30},extent = {{-10, -10}, {10, 10}}, rotation = 90)));
...
Modelica.Blocks.Interfaces.RealInput TSetCool(
  ...
  Modelica.Blocks.Interfaces.RealOutput PHeater(final quantity="HeatFlowRate",
    ...
    "Power for heating" annotation (Placement(visible = true,transformation(extent = {{160, -92}, {180, -72}}, rotation = 0),iconTransformation(extent = {{80, -80}, {100, -60}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput PCooler(final quantity="HeatFlowRate",
      final unit="W") if (not recOrSep and Cooler_on)
    "Power for cooling" annotation (Placement(visible = true,transformation(extent = {{160, -104}, {180, -84}}, rotation = 0), iconTransformation(extent = {{80, -100}, {100, -80}}, rotation = 0)));
equation
 ...
 connect(heatCoolRoom.port, thermalZoneTwoElements.intGainsConv) annotation(
    Line(points = {{152, -20}, {152, 19.25}, {92, 19.25}, {92, 20}}, color = {191, 0, 0}));
 connect(heatCoolRoom_in, heatCoolRoom.Q_flow) annotation(
    Line(points = {{152, -160}, {152, -40}}, color = {0, 0, 127}));
 ...
 connect(heaterCooler.heatCoolRoom, thermalZoneTwoElements.intGainsConv) annotation(
  ...
  connect(corGDoublePane.solarRadWinTrans, thermalZoneTwoElements.solRad)
    ...
    0,127}));
  annotation (experiment(StartTime = 0, StopTime = 31536000, Interval = 60, Tolerance = 1e-06),
